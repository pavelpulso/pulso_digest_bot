...
/**
 * Порядок: суть → ACTION (главное) → зачем → причина → ссылки. Без «Для тебя».
 */
function formatBlockText(block, postById, useHtml = true, rankMap = null) {
  ...
  const reasonLine =
    rankMap && block.ids.length === 1 && rankMap[block.ids[0]]?.reason
      ? `Почему в дайджесте: ${e(rankMap[block.ids[0]].reason)}`
      : null;
  const actionLine =
...
/** Inline-кнопки «Релевантно» / «Не релевантно» для одного поста (block.ids.length === 1). */
function feedbackKeyboard(postId) {
...
async function digestReply(ctx, offset = 0, count = DIGEST_PAGE_SIZE) {
  ...
  for (const block of blocks) {
    const blockOpts =
      block.ids.length === 1
        ? { ...optsHtml, ...feedbackKeyboard(block.ids[0]) }
        : optsHtml;
    await ctx.telegram.sendMessage(
      ctx.chat.id,
      formatBlockText(block, postById, true, rankMap),
      blockOpts
    );
  }
}
  ...
  for (const block of blocks) {
    const blockOpts =
      block.ids.length === 1
        ? { ...optsHtml, ...feedbackKeyboard(block.ids[0]) }
        : optsHtml;
    await ctx.telegram.sendMessage(chatId, formatBlockText(block, postById, true, rankMap), blockOpts);
  }
}
      ...
      if (payload) {
        ...
        for (const block of payload.blocks) {
          ...
          await botInstance.telegram.sendMessage(
            u.user_id,
            formatBlockText(block, payload.postById, true, payload.rankMap || null),
            blockOpts
          );
        }
      }
...
function profileKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("🚫 Minus words", "edit_minus_words")],
    [Markup.button.callback("📢 Рекомендация каналов", "recommend_channels")]
  ]);
}
  ...
  if (ctx.message.text.trim() === "/profile") {
    ...
    return ctx.reply(
      `Your profile (interests, profession):\n${profileText}\n\n` +
        ...
        `${minusLine}. Change: /minus_words word1, word2\n\n` +
        `Send a new profile as text to update.`
    );
  }
